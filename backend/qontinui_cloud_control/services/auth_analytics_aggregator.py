"""
Cross-user auth analytics aggregator — cloud-control admin dashboard surface.

Holds the read-only aggregation methods that compute admin-dashboard
metrics across all users (login totals, remember-me adoption rate,
device-mismatch counts, top active users, comprehensive summary). The
write path (`track_event` — recording a single event during login /
password-change flows) stays in OSS at
`app/services/auth_analytics_service.py` because every OSS-only deploy
needs to record events for its own users.

Per the carve-out principle "OSS service whose method has only
cloud-control consumers is not a layer violation, but a method whose
shape is fundamentally cross-user belongs in cloud-control": these
seven methods all aggregate or read-across users to surface in the
cloud-control admin dashboard, never on a per-user write path.

The aggregator queries OSS-side ORM models (`AnalyticsEvent`,
`DeviceSession`, `User`) directly — these are part of the canonical
schema that both halves share, so cloud-control reaching into
`app.models.*` is the same pattern cloud-control's other admin
services already use (e.g. `admin_user_repository`).
"""
...
class AuthAnalyticsAggregator:
    """Cross-user analytics aggregator backing the admin dashboard."""
...
# Singleton instance for use in cloud-control admin routes.
auth_analytics_aggregator = AuthAnalyticsAggregator()
