...
import { useNextStepSettings, type AutonomyLevel } from "./_hooks/useNextStepSettings";
  ...
  return (
    <div className="h-[calc(100vh-44px)] flex flex-col bg-background overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-border shrink-0">
        <div>
          <h1 className="text-lg font-semibold">Coordination &amp; automation</h1>
          <p className="text-xs text-muted-foreground">
            Control how coordination handles autonomous next-step work for this
            organization
          </p>
        </div>
      </div>
        ...
        <div className="max-w-2xl mx-auto px-6 py-6 space-y-6">
          ...
          {masterEnabled ? (
            <div className="flex items-start gap-3 rounded-lg border border-border bg-background px-4 py-3">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                — operator-controlled, read-only. Your settings below take
                effect when this flag is on.
              </p>
            </div>
          ) : (
            <div className="flex items-start gap-3 rounded-lg border border-yellow-500/40 bg-yellow-500/5 px-4 py-3">
              ...
              <p className="text-xs">
                ...
                <span className="text-muted-foreground">
                  You can save your preferences, but autonomous next-step will
                  not run until the operator re-enables it platform-wide.
                </span>
              </p>
            </div>
          )}
          ...
          {!canEdit && (
            <div className="rounded-lg border border-border px-4 py-3">
              <p className="text-xs text-muted-foreground">
                You have read-only access to these settings. A coord-tenant
                admin is required to make changes.
              </p>
            </div>
          )}
          ...
          <section className="space-y-4">
            <div>
              <h2 className="text-sm font-medium">Autonomous next-step</h2>
              <p className="text-xs text-muted-foreground">
              ...
              <p className="text-xs text-muted-foreground pl-0.5">
                {primaryOn ? (
                  <>
                    ...
                    when your session is stale, coordination may pick and
                    dispatch the next step (leader-gated, only while you&apos;re
                    away).
                  </>
                ) : (
                  <>
                    <span className="font-medium text-foreground">Off</span> —
                    coordination only sends you a recommendation; it never
              ...
              {primaryDomain &&
                ...
                !masterEnabled && (
                  <p className="text-xs text-yellow-500/80 pl-0.5">
                    Note: effective dispatch is also gated on the platform
                    master flag (currently off).
                  </p>
                )}
            </div>
          </section>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0 space-y-0.5">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge
                              variant={resolvedFromVariant(
                                domain.resolved_from
                              )}
                              className="text-xs"
                            >
