...
} from "@qontinui/cloud-control/services/admin/health-service";
...
export function SecurityWarningsCard({
  data,
  loading,
  onRefresh,
}: SecurityWarningsCardProps) {
  ...
  const criticalCount = unresolvedWarnings.filter(
    (w) => w.severity === "critical"
  ).length;
  const highCount = unresolvedWarnings.filter(
    (w) => w.severity === "high"
  ).length;
  const mediumCount = unresolvedWarnings.filter(
    (w) => w.severity === "medium"
  ).length;
          ...
          <ScrollArea className="h-[400px] pr-4">
            <div className="space-y-3">
              {displayWarnings.map((warning) => {
                ...
                const typeLabel =
                  warningTypeLabels[warning.type] || "Security Alert";
