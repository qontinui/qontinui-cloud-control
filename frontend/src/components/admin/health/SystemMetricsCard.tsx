...
import type { SystemMetrics } from "@qontinui/cloud-control/services/admin/health-service";
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Real-time resource usage and performance
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <Progress value={data.cpu_usage} className="h-3" />
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <Progress value={data.memory.usage_percent} className="h-3" />
              <div className="text-xs text-muted-foreground text-center">
                {data.memory.used_mb.toFixed(0)} MB /{" "}
                {data.memory.total_mb.toFixed(0)} MB
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <Progress value={data.storage.usage_percent} className="h-3" />
              <div className="text-xs text-muted-foreground text-center">
                {data.storage.used_gb.toFixed(1)} GB /{" "}
                {data.storage.total_gb.toFixed(1)} GB
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-3">
            <div className="space-y-1">
              ...
              <div className="text-xl font-bold">
                {data.database_connections.active}
              </div>
            </div>
            <div className="space-y-1">
              <div className="text-xs text-muted-foreground">Idle</div>
              <div className="text-xl font-bold">
                {data.database_connections.idle}
              </div>
            </div>
            <div className="space-y-1">
              <div className="text-xs text-muted-foreground">Max</div>
              <div className="text-xl font-bold">
                {data.database_connections.max}
              </div>
            </div>
          </div>
          <div className="space-y-1">
        ...
        {data.cpu_usage >= 90 && (
          <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/20 flex items-start gap-2">
            ...
            <div className="text-sm text-red-600">
              <span className="font-medium">CPU Critical:</span> System CPU
              usage is at {data.cpu_usage.toFixed(1)}%. Performance may be
              impacted.
            </div>
          </div>
        )}
        ...
        {data.memory.usage_percent >= 90 && (
          <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/20 flex items-start gap-2">
            ...
            <div className="text-sm text-red-600">
              <span className="font-medium">Memory Critical:</span> System
              memory usage is at {data.memory.usage_percent.toFixed(1)}%.
        ...
        {data.storage.usage_percent >= 90 && (
          <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/20 flex items-start gap-2">
            ...
            <div className="text-sm text-red-600">
              <span className="font-medium">Storage Critical:</span> Disk usage
              is at {data.storage.usage_percent.toFixed(1)}%. Free up space to
              prevent issues.
            </div>
          </div>
        )}
        ...
        {data.cpu_usage >= 75 && data.cpu_usage < 90 && (
          <div className="p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/20 flex items-start gap-2">
            ...
            <div className="text-sm text-yellow-600">
              <span className="font-medium">CPU Warning:</span> System CPU usage
              is at {data.cpu_usage.toFixed(1)}%. Monitor for continued high
              usage.
            </div>
          </div>
        )}
        ...
        {data.memory.usage_percent >= 75 && data.memory.usage_percent < 90 && (
          <div className="p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/20 flex items-start gap-2">
            ...
            <div className="text-sm text-yellow-600">
              <span className="font-medium">Memory Warning:</span> System memory
              usage is at {data.memory.usage_percent.toFixed(1)}%. Monitor usage
              closely.
            </div>
          </div>
        )}
        ...
        {data.storage.usage_percent >= 75 &&
          data.storage.usage_percent < 90 && (
            <div className="p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/20 flex items-start gap-2">
              ...
              <div className="text-sm text-yellow-600">
                <span className="font-medium">Storage Warning:</span> Disk usage
                is at {data.storage.usage_percent.toFixed(1)}%. Consider cleanup
                soon.
              </div>
            </div>
          )}
