...
class HealthService {
  ...
  async getHealthOverview(): Promise<HealthOverview> {
    const response = await fetch(
      `${ApiConfig.API_BASE_URL}/api/v1/admin/health/overview`,
    ...
    return {
      api_status:
        data.system?.alert_level === "critical"
          ? "down"
          : data.system?.alert_level === "warning"
            ? "degraded"
            : "healthy",
      database_status: data.database?.status || "healthy",
      redis_status: data.redis?.status || "healthy",
      overall_status:
        data.overall_status === "critical" ? "down" : data.overall_status,
  ...
  async getRedisStatus(): Promise<RedisStatus> {
    const response = await fetch(
      `${ApiConfig.API_BASE_URL}/api/v1/admin/health/redis`,
      this.getRequestOptions()
    );
  ...
  async getSecurityWarnings(limit: number = 50): Promise<SecurityWarning[]> {
    ...
    if (data.device_mismatches_24h > 0) {
      warnings.push({
        ...
        severity:
          data.device_mismatches_24h > 100
            ? "critical"
            : data.device_mismatches_24h > 50
              ? "high"
              : "medium",
    ...
    if (data.failed_logins_24h > 0) {
      warnings.push({
        ...
        type: "failed_login",
        severity:
          data.failed_logins_24h > 100
            ? "critical"
            : data.failed_logins_24h > 30
              ? "high"
              : "medium",
    ...
    if (data.new_devices_24h > 0) {
      warnings.push({
        ...
        type: "new_device",
        severity:
          data.new_devices_24h > 50
            ? "critical"
            : data.new_devices_24h > 20
              ? "high"
              : "low",
  ...
  async getSessionStats(): Promise<SessionStats> {
    const response = await fetch(
      `${ApiConfig.API_BASE_URL}/api/v1/admin/health/sessions`,
      this.getRequestOptions()
    );
  ...
  async getSystemMetrics(): Promise<SystemMetrics> {
    ...
    const response = await fetch(
      `${ApiConfig.API_BASE_URL}/api/v1/admin/system/health`,
      this.getRequestOptions()
    );
  ...
  downloadHealthReportCSV(data: HealthExportData): void {
    const rows = [
      ...
      ...data.security_warnings
        .slice(0, 10)
        .map((w) => [w.type, w.severity, w.message, w.timestamp]),
    ];
